...
- (ConsolidateNoteMainCollection *)collection_main {
    if (!_collection_main) {
        ...
        _collection_main = [[ConsolidateNoteMainCollection alloc] initWithFrame:(CGRect){0, 0, ZTWidth, ZTHeight-NaviIPHONEX-self.view_page.py_height} collectionViewLayout:layout];
...
- (void)requestRefresh:(BOOL)refresh
{
    ...
    if (self.questionHouse) {
        //获取本章节本题型的收藏
        [SQNetworkInterface iRequestChapterScoreDetailListParames:@{@"questionHouse":self.questionHouse,@"type":self.myNoteType,@"userId":@"33",@"page":@(page),@"rows":@"10"} andResult:^(NSInteger state, NSString * _Nonnull msg,NSString * _Nonnull total, id  _Nonnull resultData) {
            if (state == CODE_SUCCESS) {
                ...
                self.title = [NSString stringWithFormat:@"我的笔记（%@）",total];
        ...
        //获取本章节本题型的收藏
        [SQNetworkInterface iRequestConsolidateNotesParames:@{@"userId":@"33",@"page":@(page),@"rows":@"10"} andResult:^(NSInteger state, NSString * _Nonnull msg,NSString * _Nonnull total, id  _Nonnull resultData) {
            if (state == CODE_SUCCESS) {
                NSMutableArray *array = [ConsolidateMyWrongModel mj_objectArrayWithKeyValuesArray:resultData];
                self.collection_main.array_models = array;
                self.pageCount = ([total integerValue]%10)?([total integerValue]/10+1):([total integerValue]/10);
...
- (void)requestDelete:(NSMutableSet *)deleteSet
{
    if (deleteSet.count) {
        ...
        [SQNetworkInterface iRequestChapterNotesDeleteParames:@{@"ids":ids,@"userId":@"33"} andResult:^(NSInteger state, NSString * _Nonnull msg,NSString * _Nonnull total, id  _Nonnull resultData) {
            if (state == CODE_SUCCESS) {
